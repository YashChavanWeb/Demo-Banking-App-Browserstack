...
const LOCAL_URL = process.env.EXPO_PUBLIC_LOCAL_APP_URL ?? 'http://localhost:5000';
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      const [mRes, sRes, wRes, pRes] = await Promise.all([
        fetch(`${LOCAL_URL}/api/market`),
        fetch(`${LOCAL_URL}/api/stocks`),
        fetch(`${LOCAL_URL}/api/watchlist`),
        fetch(`${LOCAL_URL}/api/portfolio`),
      ]);
      ...
      setError('Cannot reach localhost:5000.\nMake sure the local-app server is running:\n  cd local-app && npm start');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
    ...
    <SafeAreaView style={styles.safe}>
      <Animated.View style={{ flex: 1, opacity: mountAnim, transform: [{ translateY: mountAnim.interpolate({ inputRange: [0, 1], outputRange: [20, 0] }) }] }}>
        ...
        <View>
          ...
          <Text style={styles.pageSubtitle}>localhost:5000 · BrowserStack Local</Text>
        </View>
      ...
      <View style={styles.body}>
        {loading ? (
          <View style={styles.center}>
            ...
            <Text style={styles.loadingText}>Connecting to localhost:5000…</Text>
          </View>
        ) : error ? (
          <View style={styles.center}>
...
const styles = StyleSheet.create({
  ...
  pageSubtitle: { color: BSColors.slate300, fontSize: 11, fontWeight: '500', marginTop: 1 },
