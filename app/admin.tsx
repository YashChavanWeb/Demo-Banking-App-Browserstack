...
export default function AdminScreen() {
  ...
  const openEdit = (u: User) => {
    setEditUser(u);
    setEditForm({ name: u.name, balance: String(u.balance), status: u.status });
  };

  const handleEdit = () => {
    if (!editUser) return;
    setUsers(p => p.map(u => u.id === editUser.id
      ? { ...u, name: editForm.name, balance: Number(editForm.balance), status: editForm.status }
      : u));
    setEditUser(null);
  };

  const handleDelete = (id: string, name: string) => {
    Alert.alert('Confirm Delete', `Remove ${name} from the system?`, [
    ...
    <SafeAreaView style={s.safe}>
      <View style={s.header}>
        <View>
          ...
          <Text style={s.headerSub}>User Management</Text>
        </View>
      ...
      <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.toolbar}>
          <View style={s.searchBox}>
            ...
            <TextInput style={s.searchInput} placeholder="Search users..." placeholderTextColor="#AAA"
        ...
        {filtered.map(u => (
          <View key={u.id} style={s.userCard}>
            <View style={s.userTop}>
              <View style={s.avatar}>
                <Text style={s.avatarText}>{u.name.split(' ').map(n => n[0]).join('')}</Text>
              </View>
              ...
              <View style={[s.badge, u.status === 'Active' ? s.badgeActive : s.badgeSuspended]}>
                <Text style={[s.badgeText, u.status === 'Active' ? s.badgeTextActive : s.badgeTextSuspended]}>
                  {u.status}
                </Text>
              </View>
            </View>
            ...
            <View style={s.userActions}>
              ...
              <TouchableOpacity
                style={[s.btnToggle, u.status === 'Active' ? s.btnSuspend : s.btnActivate]}
                onPress={() => handleToggle(u.id, u.name, u.status)} testID={`toggle-${u.id}`}>
                <Ionicons name={u.status === 'Active' ? 'pause-circle-outline' : 'play-circle-outline'} size={13}
                  color={u.status === 'Active' ? '#D97706' : '#059669'} />
                <Text style={[s.btnToggleText, { color: u.status === 'Active' ? '#D97706' : '#059669' }]}>
            ...
            <View style={s.formGroup}>
              ...
              <View style={s.typeRow}>
                {['Savings', 'Checking', 'Premium'].map(t => (
                  <TouchableOpacity key={t} style={[s.typeBtn, addForm.accountType === t && s.typeBtnActive]}
                    onPress={() => setAddForm(p => ({ ...p, accountType: t }))}>
                    <Text style={[s.typeBtnText, addForm.accountType === t && s.typeBtnTextActive]}>{t}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </View>
            <TouchableOpacity style={s.submitBtn} onPress={handleAdd} testID="submit-add">
              <Text style={s.submitBtnText}>Add User</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
        ...
        <View style={s.overlay}>
          <View style={s.modal}>
            ...
            <View style={s.formGroup}>
              ...
              <TextInput style={s.formInput} value={editForm.name}
                onChangeText={v => setEditForm(p => ({ ...p, name: v }))} testID="edit-name" />
            </View>
            <View style={s.formGroup}>
              <Text style={s.formLabel}>Balance ($)</Text>
              <TextInput style={s.formInput} value={editForm.balance}
                onChangeText={v => setEditForm(p => ({ ...p, balance: v }))}
                keyboardType="decimal-pad" testID="edit-balance" />
            </View>
            <View style={s.formGroup}>
              <Text style={s.formLabel}>Status</Text>
              <View style={s.typeRow}>
                {(['Active', 'Suspended'] as Status[]).map(st => (
                  <TouchableOpacity key={st} style={[s.typeBtn, editForm.status === st && s.typeBtnActive]}
            ...
            <TouchableOpacity style={s.submitBtn} onPress={handleEdit} testID="submit-edit">
...
const s = StyleSheet.create({
  ...
  header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#fff', paddingHorizontal: 20, paddingVertical: 16, borderBottomWidth: 1, borderBottomColor: '#F0F0F0' },
  headerTitle: { color: '#111', fontSize: 18, fontWeight: '700' },
  headerSub: { color: '#888', fontSize: 12, marginTop: 2 },
  logoutBtn: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: '#FEF2F2', paddingHorizontal: 12, paddingVertical: 7, borderRadius: 8 },
  logoutText: { color: '#DC2626', fontSize: 13, fontWeight: '600' },
  content: { paddingHorizontal: 16, paddingTop: 16, paddingBottom: 32 },
  statsRow: { flexDirection: 'row', gap: 10, marginBottom: 16 },
  statCard: { flex: 1, backgroundColor: '#fff', borderRadius: 12, padding: 12, alignItems: 'center', gap: 4, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 4, elevation: 1 },
  statValue: { fontSize: 20, fontWeight: '700' },
  statLabel: { color: '#888', fontSize: 11 },
  toolbar: { flexDirection: 'row', gap: 10, marginBottom: 16, alignItems: 'center' },
  searchBox: { flex: 1, flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', borderRadius: 10, paddingHorizontal: 12, paddingVertical: 10, gap: 8, borderWidth: 1, borderColor: '#E0E0E0' },
  searchInput: { flex: 1, fontSize: 14, color: '#111' },
  addBtn: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: BSColors.orange, borderRadius: 10, paddingHorizontal: 14, paddingVertical: 11 },
  addBtnText: { color: '#fff', fontWeight: '700', fontSize: 14 },
  userCard: { backgroundColor: '#fff', borderRadius: 14, padding: 14, marginBottom: 12, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 6, elevation: 1 },
  userTop: { flexDirection: 'row', alignItems: 'center', marginBottom: 10 },
  avatar: { width: 42, height: 42, borderRadius: 21, backgroundColor: BSColors.orange, alignItems: 'center', justifyContent: 'center', marginRight: 10 },
  ...
  formInput: { backgroundColor: '#F7F7F7', borderRadius: 10, borderWidth: 1.5, borderColor: '#E0E0E0', paddingHorizontal: 14, paddingVertical: 12, fontSize: 15, color: '#111' },
  typeRow: { flexDirection: 'row', gap: 8 },
  typeBtn: { flex: 1, paddingVertical: 9, borderRadius: 8, alignItems: 'center', backgroundColor: '#F5F5F5' },
  typeBtnActive: { backgroundColor: BSColors.orange },
  typeBtnText: { color: '#666', fontSize: 13, fontWeight: '600' },
  typeBtnTextActive: { color: '#fff' },
  submitBtn: { backgroundColor: BSColors.orange, borderRadius: 12, paddingVertical: 14, alignItems: 'center', marginTop: 4 },
